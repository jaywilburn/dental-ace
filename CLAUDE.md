...
- **Resubmit is free, and `creditChargedAt` is the only gate.** Set the first time a submission line takes its credit, stamped inside the same transaction as the decrement, and **never cleared** (not by a staff reopen, not by a company revise). Submit charges iff it is null. Only stamp when the cost is non-zero, or a free per-course event could be rejected, switched to an event-only type and resubmitted as a paid type for nothing.
- **`ensureEventDraft` must never implicitly resume a `submittedAt != null` draft.** A revision is a DRAFT whose credit is already settled; if "+ New Event" could pick it up, a provider could build an entirely different event on a paid row and submit it free. Revisions are reachable only by explicit `?eventId=`. Two implicit candidates redirect to the list rather than silently opening the wrong one.
- **Thread `?eventId=` LAST on every wizard redirect.** `lib/events/event-actions.test.ts` asserts redirect URLs with unanchored regexes that keep matching only while `eventId` is the final query parameter.
- **Two verbs for undoing a decision, and they are not interchangeable.** Staff **reopen** (`lib/reviewer/reopen-actions.ts`) means "the decision was wrong": REJECTED -> PENDING, straight back in the queue, audited as `REVIEW_REOPENED`. Company **revise** (`lib/company/resubmit-actions.ts`) means "the submission needs work": REJECTED -> DRAFT so the wizard reopens. Reopen is REJECTED-only; an APPROVED event holds a unique Event ID, a live attendee link and possibly issued certificates, so undoing an approval is a separate revoke feature, not this one.
- **`reviseEvent` must also flip session `CourseApplication`s PENDING -> DRAFT.** `rejectEvent` never touched them, and `submitEvent`'s full-course branch only moves DRAFT rows, so a FULL_EVENT_QUIZ revise throws "A session was already submitted" without it.
- **Reviewer detail pages show the whole submission without a click.** Session applications render `open` by default (`?expand=collapsed` is the opt-out), and org/contact comes from `eventOrgFields`, which coerces field by field rather than schema-parsing so one malformed email cannot blank the section. A reviewer rejected a complete 8-session event on 2026-07-29 because neither was true.
