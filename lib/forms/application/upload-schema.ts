...
/*
  Validation + path rules for course-application file uploads (presenter
  headshot only). Shared by the upload route handler and the
  uploadDraftAttachment server action (defense in depth: the route is the
  first gate, the action re-validates). Pure module — no server-only deps —
...
// Step 2 rich-text editor; "courseOutline" and "cvResume" were upload fields
// until 2026-06, when the client asked for text boxes instead. None are
// uploadable any more, but applications that captured a file still render it
// via resolveAttachmentLinks.
export const ATTACHMENT_FIELDS = ["headshot"] as const;
