/*
  ...
  - Course applications are volume-tiered per course (APP_COURSE_TIERS below)
    via the quantity-priced `app_course` SKU.
...
export type Sku = {
  ...
  grants:
    | { applicationCredits: number; certBalance?: never }
    | { applicationCredits?: never; certBalance: number };
...
export const APP_CREDIT_SKUS: Sku[] = [CATALOG.app_course];
